    ...
    <div id="projects" class="maincontent info">
        ...
        <div id="CaroWrapper">
            <div class="gameholder" v-for="card in cards" :key="card">
                ...
                <div class="arrowholder" :class="{ expand: isExpanded(card) }" @click="expandInfo(card, card)">
                    <img class="arrowl" :id="'title' + card" :class="{ expand: isExpanded(card) }" :src="arrowIcon">
                    <p v-if="!isExpanded(card)"><b>More Info</b></p>
                    <p v-else><b>Less Info</b></p>
                    <img class="arrowr" :id="'title' + card" :class="{ expand: isExpanded(card) }" :src="arrowIcon">
                </div>

                <div id="infoholder" :class="{ expand: isExpanded(card) }" v-show="isExpanded(card)">
                    <div id="gameinfo" v-if="card[0] === 'Touch Egg'">
                        <p><b>Finally, the revolutionary and culturally important egg experience comes to
                                Playdate.</b><br><br> 60 seconds on the clock, turn that crank and touch that egg! Watch
                            your fingers and don’t let Him see you, his vision is based on movement!</p>


                    </div>

                    <div id="gameinfo" v-if="card[0] === 'Monster Elevator'">
                        <p><b>As a servant for the Monster Tower Resort and Spa...</b><br><br> Your goal is to go floor
                            to floor constructing <i>premium</i> hamburger-style sandwiches for every resident. They
                            paid a pretty penny, so these particular patrons prefer prompt perfection!</p>

                    </div>

                    <div id="gameinfo" v-if="card[0] === 'Rock, Paper, Scissors, Boom!'">
                        <p><b>..Okay. So what is it?</b>
                            <br><br>Rock, Paper, Scissors, Boom! takes the concept of traditional RPS (Pro term), and
                            disregards it.
                            Shift gravity with the d-pad (or the accelerometer) to move. As you do, you create rocks,
                            papers, or scissors in your path!
                        </p>
                    ...
                    <div class="screenshots">
                        <div class="container">
                            ...
                            <div class="ssoverlay" @click="focusimage(card[3])"><img :src="magIcon"></div>
                        </div>
                        <div class="container">
                            <img class="ssimg" style="padding:10px;" :src="card[4]" @click="focusimage(screenshot)">
                            <div class="ssoverlay" @click="focusimage(card[4])"><img :src="magIcon"></div>
                        </div>
                        <div class="container">
                            <img class="ssimg" style="padding:10px;" :src="card[5]" @click="focusimage(screenshot)">
                            <div class="ssoverlay" @click="focusimage(card[5])"><img :src="magIcon"></div>
                        </div>
                    </div>
                    <a :href="card[6]" class="buybtnholder"><button class="buybtn">Buy it!</button></a>
...
const ss = ref(['']);
const cards = ref(
    [['Touch Egg', 'Will you survive the full time? Or will you become All Thumbs?', image1, ssEgg1, ssEgg2, ssEgg3, 'https://play.date/games/touch-egg/', eggTitle],
    ['Monster Elevator', 'Reach for the top as you serve up some slop!', image2, ssMe1, ssMe2, ssMe3, 'https://play.date/games/monster-elevator/', meTitle],
    ['Rock, Paper, Scissors, Boom!', 'A combustible, arcade-y twist on Rock Paper Scissors!', image3, ssRpsb1, ssRpsb2, ssRpsb3, 'https://play.date/games/rock-paper-scissors-boom!/', rpsbTitle]
    ]);
const initial_cards = cards.value;
...
function expandInfo(selectedGame, index) {
